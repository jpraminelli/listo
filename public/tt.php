...
/**
 *
 * TIMTHUMB COM UM PATCH PARA RESOLVER PROBLEMAS RELACIONADOS AO CACHE DO BROWSER.
 *
 *
 */
...
define('DIRECTORY_CACHE', './arquivos/thumbs'); // cache directory
...
if (file_exists($src)) {
    ...
    $color = imagecolorallocatealpha($canvas, 229, 223, 211, 127);
...
function get_cache_file($mime_type)
{
    ...
    if (!$cache_file) {
        // ---------------------------------------------------------------------------------------
        // PATCH para que seja possível passar um valor (microtime) no final da URL
        // para evitar problemas de cache do browser
        $query_string = explode('&', $_SERVER ['QUERY_STRING']);
        $nova_query_string = array();
        foreach ($query_string as $value) {
            if (substr($value, 0, 2) != 'mt') {
                $nova_query_string[] = $value;
            }
        }
        $nova_query_string = implode('&', $nova_query_string);
        $cache_file = DIRECTORY_CACHE . '/' . md5($nova_query_string . VERSION . filemtime($src)) . $file_type;
        // ---------------------------------------------------------------------------------------
